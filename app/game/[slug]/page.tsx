...
async function resolve(slug: string) {
  ...
  // Old numeric /game/123 links (and anything the feed re-slugs) land here.
  if (/^\d+$/.test(slug)) {
...
export default async function GamePage({ params }: PageProps<"/game/[slug]">) {
  ...
  return (
    <div
      ...
      style={{ maxWidth: "calc(11 * var(--cell) + 10 * var(--grid-gap) + 1.25rem)" }}
    >
      <script
          ...
          <Link key={c} href={`/?cat=${encodeURIComponent(c)}`} className="group">
            ...
            >
              <Icon
                ...
                className={`h-8 w-8 ${c === game.category ? "text-white" : "text-teal-950"}`}
              />
              <span
                className={`text-[11px] font-bold uppercase leading-tight ${
                  c === game.category ? "text-white" : "text-teal-600"
                }`}
              >
                {c}
              </span>
      ...
      <div className="flex flex-col gap-[var(--grid-gap)] w-full my-[var(--grid-gap)] pb-8">
        <article className="w-full rounded-[24px] bg-white p-5 sm:p-8 shadow-sm">
          <nav aria-label="Breadcrumb" className="mb-3 flex flex-wrap items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-teal-600">
            ...
            <Link href={`/?cat=${encodeURIComponent(game.category)}`} className="hover:underline">
              {game.category}
            </Link>
          </nav>
          ...
          <h1 className="text-2xl font-bold text-teal-950">
            ...
            <Link
              href="/"
              className="text-lg font-semibold text-teal-500 transition hover:text-teal-700 hover:underline"
            >
              on {SITE_NAME}
            </Link>
          </h1>
          ...
          <div className="mt-4 space-y-4 text-base leading-relaxed text-zinc-600">
            ...
            <figure className="flex justify-center py-3">
              <Image
                ...
                width={260}
                height={195}
                className="rounded-2xl shadow-[0_6px_10px_rgba(6,55,59,0.18)]"
              />
            </figure>

            {game.instructions && (
              <>
                <h2 className="pt-2 text-lg font-bold text-teal-950">
            ...
            <h2 className="pt-2 text-lg font-bold text-teal-950">
              Features of {game.title}
            </h2>
            <ul className="space-y-2 pl-5">
              <li className="list-disc">
                Plays instantly in the browser &mdash; no download, no install, no account.
              </li>
              <li className="list-disc">
                Free to play, with no time limit and no locked levels.
              </li>
              <li className="list-disc">
                A {game.category.toLowerCase()} game, so it sits alongside{" "}
                <Link
                  href={`/?cat=${encodeURIComponent(game.category)}`}
              ...
              {game.instructions && (
                <li className="list-disc">Controls: {game.instructions.toLowerCase()}</li>
              )}
              <li className="list-disc">
                Runs on desktop, tablet and phone, and expands to full screen from the button
                under the player.
              </li>
            </ul>

            <h2 className="pt-2 text-lg font-bold text-teal-950">
              Playing {game.title} on {SITE_NAME}
            </h2>
            <p>
              {game.title} runs directly in your browser &mdash; there is nothing to download
              and no account to create. Press play above and the game loads in place. Prefer
              something similar? Every game in the{" "}
              <Link
                href={`/?cat=${encodeURIComponent(game.category)}`}
                className="font-semibold text-teal-700 hover:underline"
              >
              ...
              is one click away, and the thumbnails around the player are all from that same
              category.
            </p>

            {game.tags && (
              <p className="pt-2 text-sm text-zinc-500">
                <span className="font-semibold text-teal-950">Tags: </span>
                {game.tags}
              </p>
            )}
          </div>
